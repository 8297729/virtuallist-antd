...
function VRow(props: any, ref: any): JSX.Element {
	...
	useEffect(() => {
		const initHeight = (tempRef) => {
			console.log("tempRef", { rowHeight, tempRef, totalLen });
			if (tempRef?.current && !rowHeight && totalLen) {
				// 获取实际内容高度,而不是整个tr的高度
				const contentDiv = tempRef.current.querySelector("div");
				const tempRowHeight = contentDiv?.offsetHeight ?? 0;

				vidMap.set(vid, {
					...vidMap.get(vid),
					rowItemHeight: tempRowHeight,
				});
				console.log("设置行高", tempRowHeight);
				dispatch({
					type: "initHeight",
